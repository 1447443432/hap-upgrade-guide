...
└── tools/                                ← 工具目录
    ├── md2html-tools.tar.gz             ← MD→HTML 转换工具压缩包（含编译产物+依赖）
    └── md2html/                         ← 工具解压目标目录（首次触发时自动解压）
        ├── main.go                      ← Go 源码入口（备份，ClawHub 兼容）
        └── template.html                ← HTML 模板备份（ClawHub 兼容）
```

**文件用途速查**：
- **生成升级文档时必须读取**：模板（`assets/`）+ 参考资料（`references/`）
- **转换 HTML 时优先使用**：`tools/md2html/md2html(.exe)`（首次运行自动从 `md2html-tools.tar.gz` 解压；不可用时 AI 直接转换）
...
1. **检查二进制是否已存在**：
   ...
   - 若已存在 → ✅ 跳过本步骤，直接进入 Step 1

2. **若二进制不存在，检查压缩包**：
   - 检查 `tools/md2html-tools.tar.gz` 是否存在
   - 若压缩包也不存在 → ⚠️ 输出提示"未找到 md2html 工具和压缩包，HTML 转换将使用 AI 直接转换作为 fallback"，继续执行 Step 1（不影响文档生成）
...
3. **若压缩包存在，执行解压**：
   ...
   - 若解压成功 → ✅ 进入 Step 1
   - 若解压失败（权限不足、磁盘空间、压缩包损坏等）→ ⚠️ 输出具体错误原因，提示"HTML 转换将使用 AI 直接转换作为 fallback"，继续执行 Step 1

**容错原则**：
- 本步骤**绝不阻塞**主流程。无论工具是否就绪，都必须继续执行后续步骤
- 工具不可用时的唯一影响是 HTML 转换方式降级（从 md2html 二进制 → AI 手工转换），文档内容完全一致
...
- `assets/upgrade-guide-template-standalone.md` 或 `assets/upgrade-guide-template-cluster.md`
  - 仅在需要生成升级指南时读取
  - 必须先读取模板，再填充文档
  - MD→HTML 转换优先级：① 优先使用本地 `md2html` 工具（Step 0 自检确认可用后直接调用）；② 若 Step 0 判定工具不可用，AI 直接转换（无需额外询问）
...
- 目标版本在所选架构列中的实时标记
- **当前版本发布日期**（从总表中提取）
- **目标主版本的首个版本发布日期**（从总表中提取，例如升级到 v7.3.3 时，取 v7.3.0 的发布日期）
- **目标版本发布日期**（从总表中提取，需要着重标注）
...
- **如果升级路径涉及 fileInit（文件预置数据初始化），离线场景下必须列出对应版本的 `file_init.tar.gz` 下载链接**（联网场景下 fileInit 命令直接从容器内执行，无需提前下载）
...
- 文档顶部信息必须补全升级路径、部署模式、架构、网络情况、生成日期，**必须使用两列 Markdown 表格格式**（禁止使用行内加粗键值对格式，因为会挤在一起）。格式示例：
  ...
  | **升级路径** | v4.8.1 → v7.2.1 |
  | **当前版本发布日期** | 2023-05-15 |
  | **目标主版本发布日期** | 2024-11-08 |
  | **目标版本发布日期** | **2024-12-20** ⚠️ |
...
1. **优先使用本地 `md2html` 工具**：若 Step 0 确认 `tools/md2html/` 目录下有编译好的二进制（Windows: `md2html.exe`，macOS/Linux: `md2html`），直接调用：`md2html -input xxx.md -output xxx.html`
2. **若 Step 0 判定工具不可用**：无需再次询问用户（Step 0 已输出 fallback 提示），直接用 AI 读取 MD 内容按 CSS/JS 规范手工转换并写入 `.html` 文件

**md2html 工具跨平台编译说明**（工具丢失或需重新编译时使用）：
...
- **AI 直接转换 HTML 时，严禁使用未经用户确认的本地 Python/Node.js 脚本**；优先使用项目自带的 `md2html` 工具，工具不可用时须征得用户同意再手工转换
